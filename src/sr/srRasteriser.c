...
static void putPixel(srSize posOffset, float* vertex)
{
    ...
    srPutPixel((uint)vertex[posOffset], (uint)vertex[posOffset + 1], SR_HEX_RGBA(r, g, b, a));
}

static void interpolateVertexData(float* out, srSize posOffset, srSize vertexSize, float* a,
                                  float* b, float t)
{
...
static void drawSpansBetweenEdges(srSize posOffset, srSize vertexSize, Edge* e1, Edge* e2)
{
    ...
    for (int y = e2->v1[posOffset + 1]; y < e2->v2[posOffset + 1]; ++y)
    {
        ...
        interpolateVertexData(_r.tempSpanVertex1, posOffset, vertexSize, e1->v1, e1->v2, factor1);
        _r.tempSpanVertex2[posOffset] = e2->v1[posOffset] + (int)(e2xdiff * factor2);
        _r.tempSpanVertex2[posOffset + 1] = y;
        interpolateVertexData(_r.tempSpanVertex2, posOffset, vertexSize, e2->v1, e2->v2, factor2);
...
static void drawTriangle(srSize posOffset, srSize vertexSize, float* a, float* b, float* c)
{
    // Build the edges
    Edge edges[3];
